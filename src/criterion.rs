...
fn bayes(a: &Vec<Vec<f32>>, p: &Vec<f32>) -> (f32, Vec<usize>) {
    let z = a
        .iter()
        .map(|row| {
            row.iter()
...
fn dispersion_minimization(a: &Vec<Vec<f32>>, p: &Vec<f32>) -> (f32, Vec<usize>) {
    let z_squared = a
        .iter()
        .map(|row| {
            row.iter()
...
fn probability_maximization(a: &Vec<Vec<f32>>, p: &Vec<f32>, b: f32) -> (f32, Vec<usize>) {
    let z = a
        .iter()
        .map(|row| {
            row.iter()
...
fn modal(a: &Vec<Vec<f32>>, p: &Vec<f32>) -> Option<(f32, Vec<usize>)> {
    ...
    return Some((answer, get_indeces(&z, answer)));
}
...
mod tests {
    ...
    mod risk_condition {
        use crate::criterion::{bayes, dispersion_minimization, modal, probability_maximization};
